...
export default function AdminReviewPublicationPage() {
  ...
  const fetchPublicationDetails = async () => {
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        ectsAwarded: Math.min(27, data.ectsPoints || 0),
      }));
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      setError('');
      await studentService.reviewPublication(publicationId, formData);
      navigate('/admin/publications', {
        state: { message: `Publication ${formData.isApproved ? 'approved' : 'rejected'} successfully!` },
      });
    } catch (err) {
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '2rem' }}>
          ...
          <p style={{ color: '#6b7280', marginTop: '0.5rem' }}>
            Evaluate and approve or reject this publication
          </p>
        </div>
            ...
            <div style={{ marginBottom: '2rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
                ...
                <div>
                  ...
                  >
                    {publication.isIndexedInScopus ? '✓ Yes' : '✗ No'}
                  </span>
                </div>
                ...
                <div>
                  ...
                  >
                    {publication.isIndexedInThomsonReuters ? '✓ Yes' : '✗ No'}
                  </span>
                </div>
              ...
              {formData.isApproved && (
                <div style={{ marginBottom: '1.5rem' }}>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const val = Math.min(27, Math.max(0, parseInt(e.target.value) || 0));
                  ...
                  <p style={{ fontSize: '0.875rem', color: '#6b7280', marginTop: '0.5rem' }}>
                    Publication activities are capped at 27 ECTS maximum.
                  </p>
                </div>
              )}
